...
### Declaração Literal
A declaração literal é uma forma de criar objetos diretamente no código, sem a necessidade de utilizar um construtor. Essa abordagem é especialmente aplicável a objetos como arrays, objetos literais e expressões regulares em JavaScript. Ao utilizar a declaração literal, você pode inicializar um objeto de forma concisa e legível.

##### Sintaxe
```javascript
// Declaração literal de um objeto array
const meuArray = [1, 2, 3];

// Declaração literal de um objeto literal
const meuObjeto = { chave: 'valor' };

// Declaração literal de uma expressão regular
const minhaRegExp = /padrão/;
```

Na declaração literal, os objetos podem ser criados sem a necessidade de chamar explicitamente um construtor. A sintaxe varia dependendo do tipo de objeto, mas em geral, você simplesmente define os valores desejados entre colchetes para arrays, pares de chave-valor para objetos literais e utiliza padrões para expressões regulares.

##### Exemplo
```javascript
// Utilizando a declaração literal para criar um objeto array
const novoArray = [4, 5, 6];
console.log(novoArray); // Saída: [4, 5, 6]
```

No exemplo, a declaração literal é utilizada para criar um novo array contendo os valores 4, 5 e 6. Essa abordagem torna o código mais claro e eficiente.
